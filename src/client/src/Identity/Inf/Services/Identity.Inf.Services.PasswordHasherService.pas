unit Identity.Inf.Services.PasswordHasherService;
...
type
  TPasswordHasherService = class(TService, IPasswordHasherService)
  private
...
{ TPasswordHasherService }

function TPasswordHasherService.GenerateSalt: TBytes;
begin
...
function TPasswordHasherService.Hash(const APassword: string; const ASalt: TBytes; const AIterations: Integer): TBytes;
begin
...
function TPasswordHasherService.PBKDF2_HMAC_SHA256(const APassword, ASalt: TBytes; AIterations, ADKLen: Integer): TBytes;
begin
  // n mero de blocos necess rios
...
function TPasswordHasherService.SecureCompare(const A, B: TBytes): Boolean;
begin
...
function TPasswordHasherService.Verify(const APassword: string; const AHash: TBytes; const ASalt: TBytes; const AIterations: Integer): Boolean;
begin
...
initialization
begin
  RegisterService(IPasswordHasherService, TPasswordHasherService);
end;
